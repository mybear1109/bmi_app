...
from user_data_utils import load_user_data  # 사용자 데이터 관련 함수
...
def display_ai_coach_page():
    ...
    with col2:
        ...
        restricted_exercises = [ex.strip() for ex in restricted_exercises.split(',') if ex.strip()]
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    with col1:
        if st.button("🥗 식단 계획 추천", key="diet_button"):
            with st.spinner("AI가 식단을 추천하는 중...⏳"):
                ...
                if allergen_foods:
                    additional_food_info.append(("알레르기 및 기피 음식", allergen_foods))
                if preferred_foods:
                    additional_food_info.append(("선호하는 음식", preferred_foods))
                if diet_restriction != "선택 안함":
                    additional_food_info.append(("식이 요법", diet_restriction))
                diet_plan = get_gemma_recommendation("식단", user_info, additional_food_info)
            display_diet_plan(diet_plan)
    with col2:
        if st.button("🏋️ 운동 계획 추천", key="workout_button"):
            with st.spinner("AI가 운동 계획을 추천하는 중...⏳"):
                additional_exercise_info = []
                if fitness_level != "선택 안함":
                    additional_exercise_info.append(("체력 수준", fitness_level))
                if exercise_preference:
                    additional_exercise_info.append(("선호하는 운동 유형", exercise_preference))
                if restricted_exercises:
                    additional_exercise_info.append(("제한된 운동", restricted_exercises))
                exercise_plan = get_gemma_recommendation("운동", user_info, additional_exercise_info)
