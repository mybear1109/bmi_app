...
def load_model(model_path, model_class, input_dim=13):
    ...
    try:
        ...
        st.success(f"✅ 모델 로드 성공: {model_path}")  # 관리자용 메시지 유지하고, 사용자에게는 성공 메시지로 보임
    return None
...
def load_gemma_model():
    ...
    try:
        hf_token = HF_API_KEY
        if not hf_token:
            st.error("🚨 HF_API_KEY가 설정되지 않았습니다!")  # 관리자용 메시지
            return None, None
        ...
        # 오류는 로깅 처리하고 사용자에게는 상세 오류 메시지를 숨깁니다.
        logging.error(f"🚨 Gemma 모델 로딩 중 오류 발생: {e}")
        # 사용자에게는 간단한 메시지만 표시하거나 아무 메시지도 표시하지 않습니다.
        st.info("Gemma 모델 로드 중 문제가 발생했습니다. (관리자 로그 참조)")
        return None, None
