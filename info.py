...
def display_info_page():
    ...
    st.subheader("1. google/gemma-2-9b-it모델")
    st.markdown("""
    ### 🌟 주요  특징

    - 대규모 언어 모델(텍스트 생성/Text Generation)로, 다양한 건강 관련 데이터를 학습
    - 자연어 처리 능력이 뛰어나 상황에 맞는 맥락 이해 가능
    - 기존 KoGPT2모델의 한계(한국어 변환 문제, 부적절한 답변 생성)를 극복
    - 모델 유형: 텍스트 생성(Text Generation) 모델
    - 파라미터 크기: 9B (90억) 파라미터
    ...
    - 사전 학습: 대규모 텍스트 데이터로 일반적 지식 습득
    - 파인튜닝: 건강 전문가 검증 데이터로 Hugging Face Transformers 활용
    - 프롬프트 엔지니어링: 다양한 건강 시나리오 기반 최적화
    - 온라인 학습: 실시간 데이터로 지속적 업데이트
    ...
    st.markdown("""
        ...
        - 사용자의 건강 데이터를 기반으로 AI 모델이 예측한 운동과 식단 추천 및 건강 상태 변화을 시각화합니다.
