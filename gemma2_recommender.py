...
def parse_json_response(response_json):
    """
    API 응답 객체에서 choices -> message -> content를 추출한 후,
...
def expand_allergies(allergies: List[str]) -> Set[str]:
    ...
    관련 모든 식품 목록을 반환합니다.
    """
    mapping = load_allergy_mapping()
    expanded = set()
    for allergy in allergies:
        allergy = allergy.lower().strip()
        if allergy in mapping:
            expanded.update(mapping[allergy])
        else:
            expanded.add(allergy)
    return expanded

def generate_text_via_api(prompt: str, model_name: str = "google/gemma-2b-it", retries: int = 3):
    """
    Hugging Face API의 chat completions를 사용하여 텍스트를 생성합니다.
    재시도 횟수를 지정하여 일시적인 서버 문제에 대응할 수 있습니다.
    """
    ...
    for attempt in range(retries):
        try:
            ...
            logging.error(f"🚨 API 호출 오류 (시도 {attempt+1}/{retries}): {e}")
            if attempt < retries - 1:
                time.sleep(2)  # 2초 대기 후 재시도
            else:
                st.error("🚨 API 호출 오류: 서버에 연결할 수 없습니다. 나중에 다시 시도해 주세요.")
                return {"메시지": "🚨 API 호출 오류 발생"}

def get_gemma_recommendation(category, user_info, additional_info=[]) -> Dict:
    """
    카테고리에 따라 운동 또는 식단 추천 요청 프롬프트를 구성하여 API 호출을 수행합니다.
    추가 정보(예: 알레르기, 선호 음식 등)는 튜플 리스트로 전달하며 이를 프롬프트에 포함합니다.
    """
