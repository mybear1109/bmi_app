...
def parse_json_response(response_json):
    ...
    만약 ```json 블록이 포함되어 있으면 해당 부분만 추출하여 JSON으로 파싱하고,
    그렇지 않으면 그대로 텍스트를 반환합니다.
    """
    try:
...
def get_gemma_recommendation(category, user_info, allergies=[], excluded_foods=[]):
    """
    category에 따라 운동 또는 식단 추천 요청 프롬프트를 구성하여 API 호출합니다.
    알레르기 및 제외할 음식 정보가 있을 경우 프롬프트에 추가합니다.
    """
    ...
    if category == "운동":
        ...
        prompt += "모든 대답은 반드시 한국어로 해주세요."
    elif category == "식단":
        prompt += "사용자의 건강 상태와 목표(다이어트 목적 포함)에 맞는 7일 식단 계획을 JSON 형식으로 제공해 주세요. "
        prompt += "모든 대답은 반드시 한국어로 해주세요. 특히, 다이어트를 위한 식단인 경우 칼로리 조절 및 균형 잡힌 영양소 구성이 반영되어야 합니다."
...
def get_diet_recommendation(user_info, excluded_foods):
    ...
    prompt += (
        ...
        "- 다이어트를 위한 식단은 칼로리 조절과 영양 균형(단백질, 탄수화물, 지방 비율)이 반영되어야 합니다.\n"
        "- 아침, 점심, 저녁 3끼의 식단을 추천하며, 각 식사의 영양 균형을 고려하세요.\n"
