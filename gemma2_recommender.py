...
def parse_json_response(response_json):
    ...
    try:
            ...
            print(f"json_textt 형식입니다. 응답 내용: {json_text}")
            print(f"오류 위치: {e}")
            return {f"🚨 메시지: {json_text}"}
...
def get_exercise_recommendation(user_info):
    ...
    "- 운동을 처음하시는 유저를 위해 운동방법에대한 첨부자료나 링크를 제공해주세요."
...
def get_diet_recommendation(user_info, excluded_foods):
    ...
    "- 사용자의 건강 정보를 기반으로 최적의 식단 계획을 작성해 주세요. "
    "- 일반적인 조언이 아니라, 사용자의 현재 건강 상태와 목표에 맞춘 상세한 가이드를 제공해야 합니다."
    "- 예측결과의 점수에 따라 사용자에게 식단을 추천해 주세요. "    
    "- 운동 추천 점수가 높을수록 사용자의 식단 추천 점수가 높을수록 건강한 식습관을 갖고 있다는 의미입니다."
    ...
    "- 양많고 맛있으면서 배가 부를 수 있는 식단을 추천하세요."
    "- 요리를 해야할 경우 간단하고 쉽게 만들 수 있는 요리를 추천하세요."
    "- 요리를 해야 할경우 만드는 방법도 같이 제공해주세요."
    "- 식단 예시를 포함하여 최대한 구체적으로 작성해 주세요."
    "- 매일 수행할 운동을 추천하고, 각 운동의 시간(분)과 예상 소모 칼로리를 포함하세요."
    "- 식단의 경우 목표체중을 위해 계산해서 작성해주세요."
    "- 일일 권장 칼로리를 초과하지 않도록 주의하세요."  
    "- 극단적인 다이어트나 건강에 해로운 식단은 제공하지 마세요."  
    "- 운동 부위(상체, 하체, 코어 등)를 균형 있게 고려하세요."
    "- 사용자가 피해야 할 운동(부상 위험, 건강 상태 고려)을 주의하세요."
    "- 사용자가 알러지 반응을 보이거나 피해야 하는 음식은 제외하고 식단을 작성하세요."
    "- 추천 결과를 작성할 때, 사용자의 건강을 최우선으로 고려하고, 신속하고 친절한 답변을 제공해 주세요."
    "- 식단 예시를 포함하여 최대한 구체적으로 작성해 주세요."
    "- 7일 운동 계획을 제공하세요."
    "- 아래 예시를 참고하여 작성해 주세요."
